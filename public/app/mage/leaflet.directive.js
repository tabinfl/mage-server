...
class LeafletController {
  constructor($scope, $timeout, MapService, GeometryService, LocalStorageService, EventService) {
    ...
    var FEATURE_OVERLAY_PANE = 'featureOverlayPane';
    this.FEATURE_OVERLAY_PANE = FEATURE_OVERLAY_PANE;
    map.createPane(this.FEATURE_OVERLAY_PANE);
    map.getPane(this.FEATURE_OVERLAY_PANE).style.zIndex = 399;

    var MARKER_OVERLAY_PANE = 'markerOverlayPane';
    this.MARKER_OVERLAY_PANE = MARKER_OVERLAY_PANE;
    map.createPane(this.MARKER_OVERLAY_PANE);
    map.getPane(this.MARKER_OVERLAY_PANE).style.zIndex = 599;
    ...
    function onLocation(location, broadcast) {
      ...
      if (broadcast)
        MapService.onLocation(location);
    }
    ...
    function onFeatureZoom(zoom) {
      ...
      if (!map.hasLayer(featureLayer.layer))
        return;
      if (featureLayer.options.cluster) {
        if (map.getZoom() < 17) {
          if (layer.getBounds) {
...
LeafletController.$inject = ['$scope', '$timeout', 'MapService', 'GeometryService', 'LocalStorageService', 'EventService'];