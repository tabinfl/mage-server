module.exports = {
  template: require('./saml.signin.html'),
  bindings: {
    type: '@strategyType',
    strategy: '<',
    onAuthenticate: '&'
  },
  controller: SamlSigninController
};

SamlSigninController.$inject = ['$uibModal', 'UserService'];

function SamlSigninController($uibModal, UserService) {
  ...
  this.signin = function() {
    ...
    UserService.samlSignin(strategy.name).then(function(authData) {
      ...
      $uibModal.open({
        ...
        controller: ['$scope', '$uibModalInstance', function ($scope, $uibModalInstance) {
          ...
          $scope.authorize = function (params) {
            UserService.authorize(strategy.name, user, false, {uid: params.uid}).success(function(data) {
